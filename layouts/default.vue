...
export default {
  ...
  methods: {
    ...
    switchNav (p) {
      ...
      if (p.indexOf('dashboard') >= 0) this.activeIndex = '/dashboard/me'
    }
  },
  data: function () {
    return {
  ...
  watch: {
    '$route' (to, from) {
      ...
      if (to.path.indexOf('dashboard') >= 0) {
        this.headIcon = 'logoPink'
        // document.head.appendChild(document.createElement('link').href = '~/assets/pink.scss')
      } else {
        this.headIcon = 'logoBlue'
        // document.head.appendChild(document.createElement('link').href = '~/assets/blue.scss')
      }
    }
  }
}
